...
import { ImageIcon, X, Smile, Loader2, MapPin } from "lucide-react";
...
// Loaded only when user clicks the emoji button
...
export function Feed({ initialPosts, currentUserId }: FeedProps) {
  ...
  useEffect(() => {
    ...
    setNextCursor(initialPosts.length === 10 ? (initialPosts[initialPosts.length - 1]?.id ?? null) : null);
  }, [initialPosts]);
  ...
  const onEmojiClick = (emojiData: any) => {
    ...
    const newContent = text.substring(0, start) + emojiData.emoji + text.substring(end);
        ...
        <div className="flex gap-3">
          <div className="hidden h-11 w-11 shrink-0 items-center justify-center rounded-full bg-zinc-900 border border-zinc-800 font-bold text-emerald-400 sm:flex overflow-hidden">
            {session?.user.image ? (
              ...
              <span className="h-5 w-5 text-zinc-500">?</span>
            )}
          </div>
              ...
              <div className="relative mt-3 overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-900/50">
                {isUploading ? (
                  ...
                  <>
                    ...
                    {/* Use regular img here as this is a preview of a just-uploaded image */}
            ...
            <div className="mt-3 flex items-center justify-between border-t border-zinc-800 pt-3">
              <div className="flex items-center gap-1">
                ...
                <div className="relative">
                  <button
                    onClick={() => { if (!handleActionCheck()) return; setShowEmojiPicker(!showEmojiPicker); }}
                  ...
                  {showEmojiPicker && (
                    <>
                      ...
                      <div className="absolute top-full left-0 mt-2 z-40 shadow-2xl">
                        <EmojiPicker onEmojiClick={onEmojiClick} theme={"dark" as any} autoFocusSearch={false} width={300} height={400} />
                      </div>
                    </>
                  )}
                </div>
      ...
      <div className="divide-y divide-zinc-800">
        {posts.length > 0 ? (
          ...
          <div className="flex h-[40vh] items-center justify-center px-4 text-center">
            <p className="text-base text-zinc-500 sm:text-lg">No posts yet. Be the first to share!</p>
          </div>
        )}
        ...
        {/* Sentinel div — when it enters the viewport, next page is fetched */}
